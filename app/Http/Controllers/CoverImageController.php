    ...
    public function store(Request $request)
    {
        try {
            $request->validate([
                ...
                'images.*' => 'required|image|mimes:jpg,png,jpeg,gif,svg|max:1536',
            ]);
            ...
            foreach ($images as $image) {
                $newImageName = time() . '-' . uniqid() . '-' . $image->getClientOriginalName();
                $image->move(public_path('uploads/coverimage'), $newImageName);

                $coverimage = new CoverImage;
                $coverimage->title = $request->title . ($savedCount > 0 ? ' (' . ($savedCount + 1) . ')' : '');
            ...
            if ($savedCount > 0) {
                $message = $savedCount > 1 
                    ? "Success! {$savedCount} cover images created." 
                    : 'Success! Cover image created.';
                return redirect()->route('admin.cover-images.index')->with('success', $message);
            } else {
                return redirect()->back()->with('error', 'Error! Cover images not created.');
            }
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Error! The size of the images must be less than 2MB');
        }
    }
